...
//! - Storage:
//!   - Config: ~/.claude/uira/learner.json
//!   - User skills: ~/.claude/skills/uira-learned/
//!   - Project skills: .uira/skills/
//!
//! Notes:
...
pub const PROJECT_SKILLS_SUBDIR: &str = ".uira/skills";
...
fn debug_enabled() -> bool {
    std::env::var("UIRA_DEBUG").ok().as_deref() == Some("1")
}

/// Default config path: `~/.claude/uira/learner.json`.
/// Test override: `UIRA_LEARNER_CONFIG_PATH=/tmp/.../learner.json`.
fn config_path() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("UIRA_LEARNER_CONFIG_PATH") {
        return Some(PathBuf::from(p));
    }
    home_dir().map(|h| h.join(".claude").join("uira").join("learner.json"))
}

/// Default user skills dir: `~/.claude/skills/uira-learned`.
/// Test override: `UIRA_LEARNER_USER_SKILLS_DIR=/tmp/.../uira-learned`.
fn user_skills_dir() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("UIRA_LEARNER_USER_SKILLS_DIR") {
        return Some(PathBuf::from(p));
    }
    home_dir().map(|h| h.join(".claude").join("skills").join("uira-learned"))
}
...
pub fn generate_extraction_prompt(detection: &DetectionResult) -> String {
    ...
    format!(
        "I noticed this conversation contains {} that might be worth saving as a reusable skill.\n\n**Confidence:** {}%\n**Suggested triggers:** {}\n\nWould you like me to extract this as a learned skill? Type `/uira:learner` to save it, or continue with your current task.",
        type_desc,
...
fn read_progress_raw(directory: &str) -> Option<String> {
    let (root, uira) = get_progress_paths(directory);
    if root.exists() {
        return fs::read_to_string(root).ok();
    }
    if uira.exists() {
        return fs::read_to_string(uira).ok();
    }
    None
}
...
Body"#;
    ...
    fn test_find_and_load_skills_project_overrides_user() {
        ...
        std::env::set_var("UIRA_LEARNER_USER_SKILLS_DIR", user.path());
...
Project content"#;
        ...
        std::env::remove_var("UIRA_LEARNER_USER_SKILLS_DIR");
    }
...
X"#;
    ...
    fn test_detection_hook_cooldown() {
        ...
        std::env::remove_var("UIRA_LEARNER_CONFIG_PATH");
    }

    #[test]
    fn test_promotion_candidates_from_progress() {
        let dir = tempdir().unwrap();
        let progress = dir.path().join(".uira").join(PROGRESS_FILENAME);
