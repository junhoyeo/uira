...
fn uira_storage_dir() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("UIRA_UIRA_DIR") {
        if !dir.is_empty() {
...
fn agent_usage_reminder_storage_dir() -> Option<PathBuf> {
    uira_storage_dir().map(|d| d.join("agent-usage-reminder"))
}
...
lazy_static! {
    ...
    pub static ref AGENT_TOOLS: HashSet<&'static str> =
        HashSet::from(["task", "call_omo_agent", "uira_task"]);
