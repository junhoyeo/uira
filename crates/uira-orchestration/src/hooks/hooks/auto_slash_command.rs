...
//!
//! Detects and expands `/uira:*` slash commands in user prompts.
...
use super::super::types::{HookEvent, HookInput, HookOutput, MessagePart};
...
/// Pattern to detect `/uira:*` slash commands at the start of a message.
///
/// Captures:
...
pub const SLASH_COMMAND_PATTERN: &str = r"^/(uira:[a-zA-Z][\w-]*)\s*(.*)";
...
mod tests {
    ...
    fn test_parse_slash_command() {
        let parsed = parse_slash_command("/uira:test hello world").unwrap();
        assert_eq!(parsed.command, "uira:test");
        assert_eq!(parsed.args, "hello world");
        assert_eq!(parsed.raw, "/uira:test hello world");

        assert!(parse_slash_command("not a command").is_none());
        assert!(parse_slash_command("/help").is_none()); // pattern is uira:* only
    }

    #[test]
    fn test_is_excluded_command() {
        assert!(is_excluded_command("uira:ralph"));
        assert!(is_excluded_command("UIRA:CANCEL-RALPH"));
        assert!(!is_excluded_command("uira:some-command"));
    }
    ...
    fn test_execute_slash_command_from_project_commands() {
        ...
        let cmd_path = commands_dir.join("uira:test.md");
        fs::write(
            &cmd_path,
        ...
        let parsed = ParsedSlashCommand {
            command: "uira:test".to_string(),
            args: "world".to_string(),
            raw: "/uira:test world".to_string(),
        };
        ...
        assert!(text.contains("<command-name>/uira:test</command-name>"));
    }
    ...
    async fn test_hook_integration_injects_tagged_message() {
        ...
        let commands_dir = wd.path().join(".claude").join("commands");
        fs::create_dir_all(&commands_dir).unwrap();
        fs::write(commands_dir.join("uira:test.md"), "Hi $ARGUMENTS").unwrap();
        ...
        let input = HookInput {
            ...
            prompt: Some("/uira:test there".to_string()),
            message: None,
            parts: None,
            tool_name: None,
            tool_input: None,
