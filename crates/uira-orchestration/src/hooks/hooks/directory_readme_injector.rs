...
fn readme_injector_storage_dir() -> Option<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("UIRA_README_INJECTOR_STORAGE_DIR") {
        return Some(PathBuf::from(dir));
    }

    dirs::home_dir().map(|h| h.join(UIRA_DIR).join("directory-readme"))
}
...
mod tests {
    ...
    fn test_find_readme_md_up_order_and_bounds() {
        ...
        std::env::set_var("UIRA_README_INJECTOR_STORAGE_DIR", storage.path());
        ...
        std::env::remove_var("UIRA_README_INJECTOR_STORAGE_DIR");
    }
