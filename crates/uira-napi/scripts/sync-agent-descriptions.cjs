#!/usr/bin/env node
/**
 * Syncs agent descriptions in markdown files with actual configured models from uira.yml
 *
 * This script reads the model configuration from uira.yml and updates the description
 * field in each agent's markdown frontmatter to show the actual configured model.
 */
const fs = require('fs');
const path = require('path');

const AGENTS_DIR = path.resolve(__dirname, '../../../packages/uira/claude-plugin/agents');
...
function findUiraYml() {
  let dir = process.cwd();
  while (dir !== path.dirname(dir)) {
    const configPath = path.join(dir, 'uira.yml');
...
function parseSimpleYaml(content) {
  // Simple parser for uira.yml format:
  // agents:
  //   explore:
...
function loadModelConfig() {
  ...
  if (configPath) {
    ...
    console.log('[sync-descriptions] No uira.yml found, using defaults');
  }
