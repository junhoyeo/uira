...
const PLUGIN_PKG_PATH = path.resolve(__dirname, '../../../packages/uira/native');
const PLUGIN_PKG_JSON = path.resolve(__dirname, '../../../packages/uira/package.json');

function getPluginCachePath() {
  try {
    const pkg = JSON.parse(fs.readFileSync(PLUGIN_PKG_JSON, 'utf8'));
    return path.join(os.homedir(), `.claude/plugins/cache/uira/uira/${pkg.version}/native`);
  } catch {
    return path.join(os.homedir(), '.claude/plugins/cache/uira/uira/0.1.0/native');
  }
}
...
function getPlatformBinary() {
  ...
  const directMap = {
    'darwin-arm64': 'uira.darwin-arm64.node',
    'darwin-x64': 'uira.darwin-x64.node',
    'win32-x64': 'uira.win32-x64-msvc.node',
  };
  ...
  if (platform === 'linux') {
    ...
    const muslBinary = `uira.linux-${nodeArch}-musl.node`;
    const gnuBinary = `uira.linux-${nodeArch}-gnu.node`;
...
syncTo(PLUGIN_PKG_PATH, 'packages/uira/native');
...
const AGENTS_SRC = path.resolve(__dirname, '../../../packages/uira/claude-plugin/agents');
