...
fn install_command() -> anyhow::Result<()> {
    ...
    if !Path::new(config_path).exists() {
        anyhow::bail!(
            "Config file not found: {}. Run 'uira init' first.",
            config_path
        );
    }
    ...
    for hook_name in config.hooks.keys() {
        ...
        if hook_path.exists() {
            ...
            if !existing.contains("# uira managed hook") {
...
fn generate_hook_script(hook_name: &str) -> String {
    format!(
        r#"#!/bin/sh
# uira managed hook - do not edit
# This hook was generated by uira. To modify, edit uira.yml

REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
cd "$REPO_ROOT" || exit 1

if [ -x "./target/release/uira" ]; then
    exec ./target/release/uira run {}
fi

if [ -x "./target/debug/uira" ]; then
    exec ./target/debug/uira run {}
fi

exec cargo run -q -p uira -- run {}
"#,
    ...
    if files.is_empty() || !rust_files.is_empty() {
        ...
        if !status.success() {
            if check {
                ...
                eprintln!("   Run 'uira format' to fix formatting");
            }
            process::exit(1);
        }
    }
    ...
    match action {
        ...
        AgentCommands::Info { name } => {
            ...
            if let Some(agent) = agents.get(&name) {
                ...
                anyhow::bail!(
                    "Agent '{}' not found. Use 'uira agent list' to see available agents.",
                    name
                );
            }
        }
...
fn session_command(action: SessionCommands) -> anyhow::Result<()> {
    match action {
        SessionCommands::Start { config: _ } => {
            ...
            println!();
            println!(
                "{} Session ready. Use with Claude Agent SDK or uira agent delegate.",
                "✓".green()
            );
    ...
    match action {
        ...
        SkillCommands::Show { name } => {
            if let Some(skill) = get_builtin_skill(&name) {
                ...
                anyhow::bail!(
                    "Skill '{}' not found. Use 'uira skill list' to see available skills.",
                    name
                );
            }
        }
...
async fn goals_command_async(action: GoalsCommands) -> anyhow::Result<()> {
    ...
    if !config_path.exists() {
        anyhow::bail!(
            "Config file not found: uira.yml. Run 'uira init' first or create a config with goals."
        );
    }
...
"#
    ...
    match action {
        ...
        GoalsCommands::Check { name } => {
            let runner = uira_goals::GoalRunner::new(&cwd);
        ...
        GoalsCommands::Watch {
            ...
            let options = uira_goals::VerifyOptions {
