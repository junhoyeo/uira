...
pub fn get_model_for_task(
    ...
    task_prompt: &str,
    config: RoutingConfigOverrides,
) -> (uira_sdk::ModelType, ModelTier, String) {
    if is_fixed_tier_agent(agent_type) {
        return (
            uira_sdk::ModelType::Opus,
            ModelTier::High,
...
mod tests {
    ...
    fn routes_simple_search_to_low() {
        ...
        assert_eq!(decision.model_type, uira_sdk::ModelType::Haiku);
    }
    ...
    fn orchestrator_is_fixed_opus() {
        ...
        assert_eq!(model, uira_sdk::ModelType::Opus);
