...
// Configuration
const VERSION = process.env.ASTRAPE_VERSION || '0.2.0';
const GITHUB_REPO = 'junhoyeo/astrape';
...
function getPlatformBinary() {
  ...
  const platformMap = {
    ...
    'win32-x64': 'astrape-win32-x64-msvc.exe',
  };
...
async function main() {
  ...
  if (!binaryName) {
    console.log(`[astrape] No prebuilt binary for ${process.platform}-${process.arch}`);
    console.log('[astrape] You can build from source: cargo build --release -p astrape');
    return;
  }
  ...
  if (existsSync(binaryPath)) {
    console.log('[astrape] Binary already exists, skipping download');
    return;
  }
  ...
  console.log(`[astrape] Downloading CLI binary for ${process.platform}-${process.arch}...`);
  console.log(`[astrape] URL: ${downloadUrl}`);
  ...
  try {
    ...
    console.log('[astrape] Binary installed successfully');
  } catch (err) {
    console.warn(`[astrape] Failed to download binary: ${err.message}`);
    console.warn('[astrape] You can build from source: cargo build --release -p astrape');
    console.warn('[astrape] Or download manually from:', downloadUrl);
...
main().catch((err) => {
  console.warn('[astrape] Postinstall warning:', err.message);
