/**
 * Astrape Git Hooks Module
 * 
 * Provides programmatic access to git hook functionality.
 * 
 * @example
 * ```js
 * import { install, run } from 'astrape/hook';
 * 
 * // Install git hooks
 * await install();
 * 
...
/**
 * Execute the astrape CLI with given arguments
 * @param {string[]} args - CLI arguments
 * @param {object} options - Spawn options
 * @returns {Promise<{code: number, stdout: string, stderr: string}>}
 */
function execAstrape(args, options = {}) {
  return new Promise((resolve, reject) => {
    const binPath = join(__dirname, '..', 'bin', 'astrape.js');
 ...
 * 
 * Creates hook scripts that delegate to astrape for execution.
 * Requires astrape.yml configuration file in the project root.
 * 
...
export async function install(options = {}) {
  const result = await execAstrape(['install'], {
    cwd: options.cwd,
    stdio: 'inherit',
  });
 ...
 * 
 * Executes the commands defined for the specified hook in astrape.yml.
 * 
...
export async function run(hookName, options = {}) {
  const result = await execAstrape(['run', hookName], {
    cwd: options.cwd,
    stdio: 'inherit',
  });
 ...
 * 
 * Creates a default astrape.yml configuration file if it doesn't exist.
 * 
 * @param {object} options
 * @param {string} [options.cwd] - Working directory (default: process.cwd())
 * @param {string} [options.config] - Config file path (default: 'astrape.yml')
 * @returns {Promise<void>}
 */
export async function init(options = {}) {
  ...
  const result = await execAstrape(args, {
    cwd: options.cwd,
    stdio: 'inherit',
  });
 ...
 * 
 * Runs score-based verification goals defined in astrape.yml.
 * 
 * @param {object} options
 * @param {string} [options.cwd] - Working directory (default: process.cwd())
