...
if (toolName === 'Task' && data.tool_response && uira?.notifyBackgroundEvent) {
  try {
    const response = typeof data.tool_response === 'string'
      ? JSON.parse(data.tool_response)
      : data.tool_response;
    if (response.agentId) {
      uira.notifyBackgroundEvent(JSON.stringify({
        type: 'task.completed',
...
async function runHooks() {
  try {
    ...
    const result = await uira.executeHook('post-tool-use', hookInput);
